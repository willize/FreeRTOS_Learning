...
void TIM3_Int_Init(u16 arr,u16 psc)
{
  ...
  TIM3_TimeBaseInitStructure.TIM_Period = arr; 	//自动重装载值
	TIM3_TimeBaseInitStructure.TIM_Prescaler=psc;  //定时器分频
...
void TIM4_Int_Init(u16 arr,u16 psc)
{
  ...
  TIM4_TimeBaseInitStructure.TIM_Period = arr; 	//自动重装载值
	TIM4_TimeBaseInitStructure.TIM_Prescaler=psc;  //定时器分频
	TIM4_TimeBaseInitStructure.TIM_CounterMode=TIM_CounterMode_Up; //向上计数模式
	TIM4_TimeBaseInitStructure.TIM_ClockDivision=TIM_CKD_DIV1; 
	
	TIM_TimeBaseInit(TIM4,&TIM4_TimeBaseInitStructure);//初始化TIM3
